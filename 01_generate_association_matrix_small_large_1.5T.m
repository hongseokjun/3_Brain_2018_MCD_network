...
for test_age_gender = 1
    ...
    % PMG_small   10       31.2/12.5      5/5         3/4/3           Not yet
    % PMG_large   11       26.6/9.1       4/7         3/1/7           Not yet
    
    % Confirmed: No age difference between controls and patients; between FCD Type-I and FCD Type-II
    for Age = 1
        
        % PMG_small vs. PMG_large: p=0.35
        [h,p,ci,stats] = ttest2(Age_PMG_s, Age_PMG_l, 0.05, 'both')
        
        [ mean(Age_PMG_s) std(Age_PMG_s);
          mean(Age_PMG_l) std(Age_PMG_l); ]        
    ...
    for Sex = 1
        ...
        % PMG_small vs. PMG_large: p=0.53
        [p, x2] = chisquarecont( [ sum(strcmp(Sex_PMG_l, 'male')) sum(strcmp(Sex_PMG_l, 'female')); ...
                                   sum(strcmp(Sex_PMG_s, 'male')) sum(strcmp(Sex_PMG_s, 'female')) ])
        
        [ sum(strcmp(Sex_PMG_l, 'male')) sum(strcmp(Sex_PMG_l, 'female')); ...
          sum(strcmp(Sex_PMG_s, 'male')) sum(strcmp(Sex_PMG_s, 'female')) ]       
...
for construct_asso_mat = 1
    ...
    adjacent_matrix_PMG_s               = corr(asso_mat_PMG_res_val(Size_PMG==1, :));
    adjacent_matrix_PMG_l               = corr(asso_mat_PMG_res_val(Size_PMG==2, :));    
   
    zadjacent_matrix_PMG_s              = 0.5*(log((1+adjacent_matrix_PMG_s)./(1-adjacent_matrix_PMG_s)));
    zadjacent_matrix_PMG_l              = 0.5*(log((1+adjacent_matrix_PMG_l)./(1-adjacent_matrix_PMG_l)));
    ...
    for full_conn = 1
        ...
        node_degree_uw_pos_PMG_s              = zeros(length(myspar), AAL_index_iter);
        node_degree_uw_pos_PMG_l              = zeros(length(myspar), AAL_index_iter);  
        node_strength_uw_pos_PMG_s            = zeros(length(myspar), AAL_index_iter);
        node_strength_uw_pos_PMG_l            = zeros(length(myspar), AAL_index_iter);  
        ...
        for i = 1 : length(myspar)
            ...
            node_degree_uw_pos_PMG_s(i, :)             = degrees_und(double(wmatrix>0));
            node_strength_uw_pos_PMG_s(i, :)           = strengths_und(double(wmatrix>0));
            
            wmatrix                                    = threshold_proportional(zadjacent_matrix_PMG_l, myspar(i));
            node_degree_uw_pos_PMG_l(i, :)             = degrees_und(double(wmatrix>0));
            node_strength_uw_pos_PMG_l(i, :)           = strengths_und(double(wmatrix>0));
            
            %% check if fully connected
            if((sum(node_degree_uw_pos_PMG_s(i, :)==0)+sum(node_degree_uw_pos_PMG_l(i, :)==0)) > 0)
    ...
    if(printfigs == 1)
        ...
        plotCorrelationMatrix(zadjacent_matrix_PMG_s,{},[-1 1]); 
        ...
        ax1 = axes('Position',[0 0.1100 0.92 0.8150],'Visible','off'); 
        ax2 = axes('Position',[0.1129 0.1100 0.6731 0.8150]); axes(ax2);
        plotCorrelationMatrix(zadjacent_matrix_PMG_l,{},[-1 1]); 
        hold on; plot(vertical_line_idx_x, vertical_line_idx_y, 'Color', [1 1 1], 'LineWidth', 2);
        hold on; plot(vertical_line_idx_y, vertical_line_idx_x, 'Color', [1 1 1], 'LineWidth', 2);
        set(gca, 'TickLength', [ -0.03 -0.03 ] );
        set(gca, 'YTick', struct_idx );
    ...
    %% save variables
    if(exist([ MATDIR '/01_generate_association_matrix_PMG_small_vs_large.mat' ])~=2)
        save([ MATDIR '/01_generate_association_matrix_PMG_small_vs_large.mat' ], ...
              'adjacent_matrix_PMG_s', 'adjacent_matrix_PMG_l', ...
              'zadjacent_matrix_PMG_s', 'zadjacent_matrix_PMG_l', ...
              'Anatomical_Label_structs', 'Anatomical_Label_structs_idx', ...
              'asso_mat_PMG_res_val',...
              'asso_mat_PMG_val');
    end
...
for statistical_test = 1
    ...
    for original_AAL = 1
        ...
        %% 1) PMG_small vs. PMG_large
        group1 = zadjacent_matrix_PMG_s;
        group2 = zadjacent_matrix_PMG_l;
        ...
        diff_real_wmatrix_PMG_small_vs_large = diff_real_wmatrix;
        pmap_mat_cont_vs_PMG_small_vs_large = pmap_mat;
        ...
        if(printfigs == 1)
            ...
            %% PMG small vs. PMG large
            pmap_mat            = pmap_mat_cont_vs_PMG_small_vs_large(:, :, spar);
            diff_real_wmatrix   = diff_real_wmatrix_PMG_small_vs_large(:, :, spar);
