...
for test_age_gender = 1
    ...
    % PMG          21      28.8/11.2     12/9         6/6/9            Not yet   
    % Total     132 (41/91)
    ...
    % Confirmed: No sex difference between controls and FCD_Type_I; between controls and FCD; between FCD_Type_I and FCD
    for Sex = 1
...
for construct_asso_mat = 1
    ...
    for regress_out_nuisance = 1
        ...
        %% FCD Type I
        Sex_group = [ Sex_FCD_Type_I; ];
        Age_group = [ Age_FCD_Type_I; ];
        T         = [ T_FCD_Type_I_whole; ];
        mean_T = zeros(size(T, 1), 1);
        ...
        for i = Anatomical_Label_structs_idx
            ...
            ROI_thick = [ asso_mat_FCD_Type_I_val(:, count); ];
            slm = SurfStatLinMod(ROI_thick, model);
            ROI_res = ROI_thick - slm.X*slm.coef;            
            
            asso_mat_FCD_Type_I_res_val(:, count)     = ROI_res;
            microAAL_idx = unique(microAAL_surf_data_both(AAL_surf_data_both == i));
            
            for j = 1 : size(microAAL_idx, 2)
                
                ROI_thick = [ micro_asso_mat_FCD_Type_I_val(:, count2); ];
                slm = SurfStatLinMod(ROI_thick, model);
                ROI_res = ROI_thick - slm.X*slm.coef;
                
                micro_asso_mat_FCD_Type_I_res_val(:, count2) = ROI_res;
    ...
    %% save variables
    if(exist([ MATDIR '/01_generate_association_matrix_MCD2.mat' ])~=2)
        save([ MATDIR '/01_generate_association_matrix_MCD2.mat' ], ...
    ...
    if(exist([ MATDIR '/01_generate_micro_association_matrix_MCD2.mat' ])~=2)
        save([ MATDIR '/01_generate_micro_association_matrix_MCD2.mat' ], ...
